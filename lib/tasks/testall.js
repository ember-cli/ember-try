...
module.exports = CoreObject.extend({
  ...
  _testVersion: function(scenario){
    var task = this;
    var stdoutFileName = scenario.stdoutTestall;
    if (typeof stdoutFileName  !== 'undefined') {
      stdoutFileName = path.join(this.project.root, stdoutFileName);
    }
    return this.ScenarioManager.changeTo(scenario)
      .then(function(){
        return task._runTests(stdoutFileName);
      });
  },

  _runTests: function(stdoutFileName){
    var task = this;

    return findEmberPath(task.project.root)
      .then(function(emberPath){
        return run('node', [emberPath, 'test'], {cwd: task.project.root}, stdoutFileName)
      })
      .then(function(){
