...
module.exports = CoreObject.extend({
  ...
  _restoreOriginalDependencies() {
    ...
    if (fs.existsSync(yarnLockBackupFileName)) {
      restoreTasks.push(
        copy(yarnLockBackupFileName, path.join(this.cwd, this.yarnLock))
      );
    }
    ...
    if (fs.existsSync(npmShrinkWrapBackupFileName)) {
      restoreTasks.push(
        copy(npmShrinkWrapBackupFileName, path.join(this.cwd, this.npmShrinkWrap))
      );
    }
    ...
    if (fs.existsSync(packageLockBackupFileName)) {
      restoreTasks.push(
        copy(packageLockBackupFileName, path.join(this.cwd, this.packageLock))
      );
    }
  ...
  _backupOriginalDependencies() {
    ...
    if (fs.existsSync(nodeModulesPath)) {
      backupTasks.push(
        copy(nodeModulesPath, path.join(this.cwd, this.nodeModulesBackupLocation), { clobber: true })
      );
    }
    ...
    if (fs.existsSync(yarnLockPath)) {
      backupTasks.push(
        copy(yarnLockPath, path.join(this.cwd, this.yarnLockBackupFileName))
      );
    }
    ...
    if (fs.existsSync(packageLockPath)) {
      backupTasks.push(
        copy(packageLockPath, path.join(this.cwd, this.packageLockBackupFileName))
      );
    }
