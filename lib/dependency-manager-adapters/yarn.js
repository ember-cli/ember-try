...
module.exports = CoreObject.extend({
  ...
  cleanup: function() {
    ...
    return adapter._restoreOriginalDependencies().then(function() {
      debug('Remove backup package.json, node_modules and yarn.lock');
      return RSVP.all([rimraf(path.join(adapter.cwd, adapter.packageJSONBackupFileName)),
        rimraf(path.join(adapter.cwd, adapter.yarnLockBackupFileName)),
        rimraf(path.join(adapter.cwd, adapter.nodeModulesBackupLocation))]);
    }).catch(function(e) {
      console.log('Error cleaning up yarn scenario:', e);
    })
      .then(function() {
  ...
  _install: function() {
    ...
    debug('Run yarn with options %s', options);
  ...
  _restoreOriginalDependencies: function() {
    ...
    debug('Restoring original package.json, node_modules and yarn.lock');

    return RSVP.all([
      copy(path.join(this.cwd, this.packageJSONBackupFileName),
        path.join(this.cwd, this.packageJSON)),
      copy(path.join(this.cwd, this.yarnLockBackupFileName),
  ...
  _backupOriginalDependencies: function() {
    ...
    debug('Backing up package.json, node_modules and yarn.lock');

    return RSVP.all([
      copy(path.join(this.cwd, this.packageJSON),
        path.join(this.cwd, this.packageJSONBackupFileName)),
      copy(path.join(this.cwd, this.yarnLock),
