...
module.exports = CoreObject.extend({
  ...
  options: {
    /**
     * Set from config/ember-try.js' pnpmUseLockfile
     */
    useLockfile: false,
  },
  ...
  async setup() {
    ...
    if (fs.existsSync(lockFile)) {
      debug(`Backing up ${PNPM_LOCKFILE}`);

      if (this.options.useLockfile) {
        await fs.copy(lockFile, lockFileBackup);
      } else {
        /**
         * Unlike the other adapters, we *move* the lockfile, rather than copy
         * This is, in part, because pnpm generates generally more stable lockfiles
         * than the other package managers, but also because of default update behavior
         * implemented since pnpm 8, where pnpm is optimizing install speed instead of the
         * "latest" behavior (when a lockfile is present). This is to encourage folks to
         * manage their dependencies via their package.json, rather than only via their lockfile
         * (you upgrade a dependency, you make it intentional via package.json).
         * When there is no lockfile, pnpm will then install the "latest" versions of each
         * dependency matching the specified semver range -- this simulates the "floating deps"
         * behavior that we want to ensure happens when using ember-try
         * -- this simulates a "new user install".
         *
         * Additional information on pnpm's behavior here:
         * - https://github.com/pnpm/pnpm/issues/6463#issuecomment-1525839756
         * - https://github.com/pnpm/pnpm/issues/6463#issuecomment-1536115297
         */
        await fs.move(lockFile, lockFileBackup);
      }
    }
  },
