...
module.exports = CoreObject.extend({
  print: function() {
    ...
    this.results.forEach(function(scenario, index) {
      if (scenario.result) {
        colorAndMessage = chalk.green('Scenario ' + scenario.scenario + ': PASS');
        countPassed++;
      } else {
  ...
  _printDependencyTable: function(dependencyStatus) {
    ...
    var tableRow;
    var table = new Table({
      head: [chalk.gray('Dependency'), chalk.gray('Expected'), chalk.gray('Used')], colWidths: [20, 20, 30]
    });
    dependencyStatus.forEach(function(dep, index) {
      ...
      tableRow = [dep.name, dep.versionExpected, dep.versionSeen].map(function(column) {
  ...
  _printResultsSummary: function(countFailed, countPassed, total) {
    ...
    if (countFailed) {
      ...
      task.ui.writeLine(chalk.green(countPassed + ' scenarios passed'));
      task.ui.writeLine(chalk.gray(total + ' scenarios run'));
    } else {
      task.ui.writeLine(chalk.green('All ' + countPassed + ' scenarios passed'));
    }
  }
});
