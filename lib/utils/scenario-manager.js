...
module.exports = CoreObject.extend({
  changeTo: function(scenario) {
    ...
    return NpmHelpers.resetNpmFile(manager.project.root)
      ...
      .then(function() {
        ...
        npmJSON.dependencies = npmJSON.dependencies || {};

        scenario.npm = scenario.npm || {dependencies: {}, devDependencies: {}};
  ...
  _checkVersions: function(scenario, packageManagerKey) {
    ...
    pkgs.map(function(dep) {
      ...
      if (actualVersion !== expectedVersion) {
        manager.ui.writeLine(Chalk.yellow('Versions do not match: Expected: ' + expectedVersion +
          ' but saw ' + actualVersion + ' This might be ok, depending on the scenario'));
      }
  ...
  _overrideManifestJSONDependencies: function(base, scenario, kindOfDependency) {
    ...
    pkgs.forEach(function(pkg) {
      if (scenario[kindOfDependency][pkg]) {
        base[kindOfDependency][pkg] = scenario[kindOfDependency][pkg];
      } else {
        delete base[kindOfDependency][pkg];
      }
